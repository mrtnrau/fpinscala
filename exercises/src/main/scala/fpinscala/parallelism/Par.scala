...
object Par {
  ...
  def parFilter[A](as: List[A])(f: A => Boolean): Par[List[A]] = {
    val bs: List[Par[List[A]]] = as.map(asyncF(a => if (f(a)) List(a) else Nil))
    map(sequence(bs))(_.flatten)
  }

  def fold[A](as: IndexedSeq[A])(zero: A)(combine: (A, A) => A): A =
    if (as.size <= 1)
      as.headOption.getOrElse(zero)
    else {
      val (l, r) = as.splitAt(as.length / 2)
      combine(fold(l)(zero)(combine), fold(r)(zero)(combine))
    }
}
